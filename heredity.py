...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    genes = 0
    for person in people:
        # check whether people in one gene or two genes or none of them
        ...
        # if the person has parents listed
        else:

            # check and calculate mother's probability to pass the gene to the person
            if people[person]["mother"] in one_gene:
                mother_prob = (1 - PROBS["mutation"]) * 0.5
            elif people[person]["mother"] in two_genes:
                mother_prob = 1 - PROBS["mutation"]
            else:
                mother_prob = PROBS["mutation"]

            # check and calculate father's probability to pass the gene to the person
            if people[person]["father"] in one_gene:
                father_prob = (1 - PROBS["mutation"]) * 0.5
            elif people[person]["father"] in two_genes:
                father_prob = 1 - PROBS["mutation"]
            else:
                father_prob = PROBS["mutation"]

            # calculate the probability that the person has one gene or two genes or no gene based on the list they belong to
            if genes == 1:
        ...
        # Calculate the probability a person have trait or don't have trait
        if person in have_trait:
            trait_probability = PROBS["trait"][genes][True]
        else:
            trait_probability = PROBS["trait"][genes][False]

        p = p * gene_probability * trait_probability
    return p
...
def normalize(probabilities):
    ...
    for person in probabilities:
        ...
        # norminalize gene distribution
        sum_gene = probabilities[person]["gene"][1] + probabilities[person]["gene"][2] + probabilities[person]["gene"][0]
        probabilities[person]["gene"][1] /= sum_gene
        probabilities[person]["gene"][2] /= sum_gene
        probabilities[person]["gene"][0] /= sum_gene

        # norminalize trait distribution
